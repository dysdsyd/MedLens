...
# Main Run Function
# ********************
mob_run <- function(file_path, formula, id_key='IMS_ID', train_split=0.8, max_depth=4, min_size=50, ver_string = 'default'){
  setwd('D:/Anaconda3/projects/')
  result_path <-file.path(paste0('Collegium_Pharma/04_Results/', ver_string))
  ...
  system.time(model <- TrainMOBModel(dat, formula, fs, idx, max_depth = max_depth, min_size = min_size))
  saveRDS(model, paste0(result_path, '/MOB_model.rds'))
  # Plotting Tree ------------------------------------------------------------
  jpeg(filename = paste0(result_path,'/box_tree.jpg'),width=8800,height=6950,res=300)
  plot(as.constparty(model))
  dev.off() 
  jpeg(filename = paste0(result_path,'/scatter_tree.jpg'),width=8800,height=6950,res=300)
  plot(model)
  dev.off()
  
  GetMasterData(model, dat, fs, result_path)
...
GetMasterData <- function(model, dat, fs, result_path){
...
TrainMOBModel <- function(dat, formula, fs, idx, max_depth = 4, min_size = 50){
  model <- glmtree(formula, data = dat[idx$train, ], family = "gaussian", 
                   alpha = .05, epsilon = 1e-4, maxit = 100, vcov = 'info',
